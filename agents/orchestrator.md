...
**用实验的方法做事**：多方案并行尝试 + 短/长期规划 + 数据分析。不是"想明白了再改"，是"有假设就并行试几个看哪个真"。

Mentor 的建议、Codex 的诊断、测试通过的 pytest、自己的推理 —— **都只是假设**。真相只在**跑完 C 的 evidence + 真实 e2e 数字**里。

- 派 Codex 改完 → **必须派 C 跑**（或等价 smoke），分数 + evidence 未验证前不算改完
- Mentor 说的根因 → **必须执行后再看**（评分是否动、方向是否对），回来再给 Mentor 看后果
- Codex 自称 "pytest 通过" → **不够**。pytest 测试的是 Codex 写的测试，不是真实场景
- 我自己推理的"这应该没问题" → **最不可信**，永远低估盲区

工作口诀：**假设 → 执行 → 校验 → 纠正**。跳过校验等于拿用户信任赌运气。

上一小时的惨痛教训：Codex 2 大重构跑 2 个 pytest 通过就合入，E2E 没跑 → T1/T2 "AI 90s 零响应 + composer 永久 disabled" P0 灾难。规矩：> 200 LOC 产品重构必须前置 T1/T2/T7 happy-path smoke gate，否则不合入。
...
6. **带后果回 Mentor**：Mentor 给建议 → 执行 → 测试 → 把**执行结果 + 测试数据**回传给下一轮 Mentor 反思（见 memory `feedback_mentor_feedback_loop.md`）。Mentor 不是神谕，是会学习的顾问。
